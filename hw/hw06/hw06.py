...
def two_list(vals, counts):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(curr_count, curr_index):
        if curr_count - 1 == 0:
            if curr_index == len(vals) - 1:
                return Link(vals[curr_index])
            else:
                return Link(vals[curr_index], helper(counts[curr_index + 1], curr_index + 1))
        else:
            return Link(vals[curr_index], helper(curr_count - 1, curr_index))
