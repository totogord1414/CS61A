def shuffle(s):
    ...
    s1 = s[:len(s) // 2]
    s2 = s[len(s) // 2:]
    return sum(([s1[i], s2[i]] for i in range(len(s) // 2)), [])
...
def balanced(m):
    ...
    left_arm = left(m)
    right_arm = right(m)
    left_mobile = end(left_arm)
    right_mobile = end(right_arm)
    return balanced(left_mobile) and balanced(right_mobile) and total_mass(left_mobile) * length(left_arm) == total_mass(right_mobile) * length(right_arm) 
...
def prune_leaves(t, vals):
    ...
    return tree(label(t),[branch for branch in b if branch != None])
